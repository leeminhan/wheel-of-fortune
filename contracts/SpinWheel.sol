...
contract SpinWheel {
    ...
    function startGame() public returns (uint256 _gameId, string memory _topic){
        ...
        console.log(currentGameId, topic);
        return (currentGameId, topic);
    }

    /**
        Create mapping to track User account balance
        Create admin mapping
        -----
        1. Deploy contract
        2. Start game: {0 : GameStatus(address, OPEN, topic)} -> gameId to be used in next transaction & topic for user to guess
        3. GuessByWord(uint gameId, string guess)
            1. Fetch the GameStatus using the provided gameId
            2. Check msg.sender matches the GameStatus's address
            3. Check if GameState still OPEN, else, revert invalid
            4. Check if the guess matches the random generated word succesfully (1 GUESS ONLY)
                1. Update GameState to CLOSE and return corresponding reward
            5. Update user balance
            // addressToBalance[msg.sender] = reward;
    ...
    function guessByWord(uint256 _gameId, string memory guess) public {
        ...
        string memory correctAnswer = gameStatus.topic;
        uint256 rewardValue;
        
        if (keccak256(abi.encodePacked((guess))) == keccak256(abi.encodePacked((topicToWord[correctAnswer])))) {
